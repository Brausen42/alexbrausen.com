<template>
  <div class="spotify-component" v-on:mouseover="updateHeaderHeight()">
    <div v-if="!playlist.error">
      <div class="centered" id="upcomingHeader">
        <img
          id="headerImage"
          v-bind:style="headerStyleObject"
          v-bind:src="playlist.imageUrl"
          alt="Playlist image"
        >
        <div id="headerDiv">
          <h2>{{ playlist.name }}</h2>
...
export default Vue.extend({
  ...
  methods: {
    updateHeaderHeight: function() {
      ...
      this.headerHeight = headerDiv
        ? `${headerDiv.offsetHeight}px` || "100px"
        : "100px";
    }
  },
  created: async function() {
    ...
    await get(playlistTrackOptions)
      .then((res: any) => {
        self.playlist = res;
